...
def connecttoRedis():
    import redis 
    redis_client=redis.Redis(host='redisgraph',port=9001,charset="utf-8", decode_responses=True)
    return redis_client
...
def process_item(record):
    ...
    if debug:
        ...
        log("""MERGE (source: entity { id: '%s', label :'entity', name: '%s',role: '%s'}) 
         ON CREATE SET source.rank=1
         ON MATCH SET source.rank=(source.rank+1)
         MERGE (destination: entity { id: '%s', label: 'entity', name: '%s',role: '%s' })
         ...
         ON MATCH SET r.rank=(r.rank+1)""" % (source_id ,source_name,role,destination_id,destination_name,role,year))
    response=rconn.execute_command("GRAPH.QUERY", "cord19medical","""MERGE (source: entity { id: '%s', label :'entity', name: '%s', role: '%s'}) 
         ON CREATE SET source.rank=1
         ON MATCH SET source.rank=(source.rank+1)
         MERGE (destination: entity { id: '%s', label: 'entity', name: '%s', role: '%s' })
         ON CREATE SET destination.rank=1
